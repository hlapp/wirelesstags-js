...
/**
 * Obatins list of positive capabilities (canXXX()) and facts (isYYY(),
...
/**
 * Saves the current tag information to the cloud. Specifically, saves changes
 * to properties `name`, `comment`, and other r/w properties.
 *
 * @param {module:wirelesstags~apiCallback} [callback]
 * @returns {Promise} Resolves to this tag object once the save completes.
 */
WirelessTag.prototype.save = function(callback) {
    var tagRec = Object.assign({}, this.data);
    ['ampData', 'thermostat', 'thermostatRef'].forEach((k) => delete tagRec[k]);
    var req = this.callAPI(
        '/ethClient.asmx/SaveTagInfo',
        { tag: tagRec },
        callback);
