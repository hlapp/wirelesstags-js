...
function WirelessTag(tagManager, tagData) {
    ...
    // this.on('data', (tag) => { console.log(tag.data); });
    this.data = tagData || {};
    this.on('data', this.discoverSensors.bind(this));
    this.on('data', this.bounceUpdateLoop.bind(this));
    this.on('update', (obj, propName, value) => {
        if (propName == 'updateInterval') return setUpdateInterval(this, value);
    });
}
...
WirelessTag.prototype.update = function(tag, retry, callback)  {
    if ((tag === undefined) || (tag === true) || (tag === false)) {
        retry = tag;
        tag = this;
    }
    if (retry) {
        if (tag._updateAttempts === undefined) {
            tag._updateAttempts = 0;
        } else {
            tag._updateAttempts++;
        }
    }
    ...
    return req.then(
        (result) => {
            if (tag.data.lastComm == result.lastComm) {
                // data not updated by tag manager, retry if requested
                if (retry) {
                    if (this._updateAttempts >= MAX_UPDATE_RETRIES) {
                        return Promise.reject("no update from tag", tag.slaveId,
                                              "after", MAX_UPDATE_RETRIES,
                                              "attempts");
                    } else {
                        console.warn("## retrying update for tag", tag.slaveId);
                        return new Promise((resolve,reject) => {
                            let action = () => {
                                tag.update(retry, callback).then(
                                    (result) => { resolve(result); },
                                    (error) => { reject(error); }
                                );
                            };
                            setTimeout(action, RETRY_UPDATE_DELAY);
                        });
                    }
                }
            }
...
WirelessTag.prototype.toString = function() {
    ...
    for (let cap in this) {
        if (cap.startsWith("can")
            ...
            if ((('function' === typeof propValue) && propValue.call(this))
                || (('function' !== typeof propValue) && propValue)) {
                propsObj.capabilities.push(cap);
            }
        }
    }
...
function setUpdateInterval(tag, value, callback) {
    if (('number' !== typeof value) || (value <= 0)) {
        return Promise.reject("invalid update interval for tag", tag.name);
    }
    var req = tag.callAPI(
        '/ethClient.asmx/SetPostbackIntervalFor',
        { id: tag.slaveId, sec: value },
        callback);
    return req.then(
        (result) => {
            tag.data = result;
            if (tag.updateInterval != value) {
