...
const monitoringPropertiesMap = {
    ...
    'outofrange': {
        notifySettings: [
            ...
            xforms.revMapObjectFunction(objectMaps.notifyOutOfRangeMap)],
    },
};
...
WirelessTagSensor.prototype.arm = function(callback) {
    ...
    if (! this.canArm()) {
        return Promise.reject(
            new OperationUnsupportedError(this.sensorType
        ...
        (result) => {
            ...
            if (sensor.sensorType === 'motion'
                ...
                Object.defineProperty(config, 'gracePeriod', {
                    ...
                    get: xforms.delegatingFunction(sensor, 'gracePeriod'),
                    set: (newValue) => {
                        xforms.delegatingFunction(sensor,
...
MonitoringConfig.saveFunc = function(sensor) {
    return function(callback) {
        if (this.isModified()) {
            // if this is an out or range config and grace period is one of the
            ...
            if (this.isModified('gracePeriod')) {
                setGrace = WirelessTagSensor.setOutOfRangeGracePeriod(this);
            } else {
                setGrace = Promise.resolve(sensor);
            }
            // then save the (possibly rest of) monitoring config
            let req = setGrace.then(
                (s) => {
                    WirelessTagSensor.setMonitoringConfig(sensor,
                                                          this,
                                                          callback);
                },
                sensor.errorHandler(callback)
