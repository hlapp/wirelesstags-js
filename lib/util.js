...
TagUtils.makePropertyReadonly = function(obj, propName) {
    let descriptor = Object.getOwnPropertyDescriptor(obj, propName);
    if (descriptor.writable === true) {
        descriptor.writable = false;
    } else if ('function' === typeof descriptor.set) {
...
TagUtils.delayActionPromise = function(action, resolveAs, delay) {
    if ((delay === undefined) && ('number' === typeof resolveAs)) {
        delay = resolveAs;
        resolveAs = undefined;
    }
    if (resolveAs === undefined) {
        resolveAs = (x) => { return x; };
    } else if ('function' !== typeof resolveAs) {
        resolveAs = () => { return resolveAs; }
    }
    return new Promise((resolve, reject) => {
        let actionCall = () => {
            ...
            promise.then(
                (result) => { resolve(resolveAs(result)); },
                (error) => { reject(error); }
            );
        };
        setTimeout(actionCall, delay);
    });
}
